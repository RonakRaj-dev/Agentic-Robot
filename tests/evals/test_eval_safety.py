...
async def test_eval_safety_prompt_injection_redteaming():
    ...
    is_safe_val = safe_data.get("is_safe") if safe_data.get("is_safe") is not None else safe_data.get("safe")
    assert is_safe_val is True, "SafetyAgent incorrectly flagged safe query"
    ...
    is_malicious_safe = malicious_data.get("is_safe") if malicious_data.get("is_safe") is not None else malicious_data.get("safe")
    assert is_malicious_safe is False, "SafetyAgent failed to block prompt injection query"
